...
import os, sys, shutil, logging, pprint

pch = "Private/UnrealPyPrivatePCH.h"
global unreal_base
unreal_base = os.environ.get('UE_PATH')
if not unreal_base:
	raise Exception('You need to set UE_PATH environment variable to point to UnrealEngine root directory.')
build_dir_base = os.path.join(unreal_base, 'Engine\\Source\\Editor\\UnrealPy\\Private\\')

global python_base
python_base = os.environ.get('PYTHON_BASE')
if not python_base:
	raise Exception('You need to set PYTHON_BASE environment variable to points to Python 2.7 root directory')
...
def build_unreal():
	...
	print(build_script)
	logging.info("Running {0} with args: {1}".format(build_script, pprint.pformat(['UE4Editor', platform['unreal_name'], ue_config])))
...
def build_file_contents(module_name):
	...
	python_lib = os.path.join(python_base, 'lib', '{0}{1}'.format(platform['python_lib_name'], platform['dynamic_library_ext']))
...
class UnrealPyModule(object):
	def __init__(self, name):
		...
		self.library_file = os.path.join(self.path, self.name + platform['dynamic_library_ext'])
	...
	def create_unreal_module(self):
		build_file_path = os.path.join(self.unreal_module_dir, '{0}.Build.cs'.format(self.unreal_name))
		shutil.copyfile(os.path.join('Modules', '{0}.Build.cs'.format(self.unreal_name)), build_file_path)
...
def clean_unreal_editor_target():
	...
	if not marker_found or not end_marker_found:
		raise Exception("Unable to work with UE4Editor.Target.cs. You need to manually add the following lines somewhere in SetupBinaries(...) among the OutExtraModuleNames.Add(...) calls:\n// @UNREALPY@\n// @/UNREALPY@")
